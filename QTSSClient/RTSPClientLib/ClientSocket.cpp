 ...
 : fSocket(NULL, inSocketType)
{
	...
	fSocket.SetSocketBufSize(20480);//fym 设置发送缓冲区,接收缓冲区已在ClientSession::Run()中设置
}
...
OS_Error TCPClientSocket::SendV(iovec* inVec, UInt32 inNumVecs)
{
    if (fSendBuffer.Len == 0)
    {
        for (UInt32 count = 0; count < inNumVecs; count++)
        {
            ...
            //fym Assert(fSendBuffer.Len < ClientSocket::kSendBufferLen);
        }
    }
...
//fym
//data必须已添加RTSP包头
OS_Error TCPClientSocket::send_media_data(const char* data, const unsigned short length, const unsigned char type)
{
	if(NULL == data || 4 >= length || 2 < type)
		return OS_NoErr;
	...
	return fSocket.Send(data, length, &sent_len);
}
