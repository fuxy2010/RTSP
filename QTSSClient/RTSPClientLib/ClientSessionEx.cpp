...
UInt32          ClientSessionEx::sActiveConnections = 0;
UInt32          ClientSessionEx::sPlayingConnections = 0;
UInt32          ClientSessionEx::sTotalConnectionAttempts = 0;

UInt32          ClientSessionEx::sBytesReceived = 0;
UInt32          ClientSessionEx::sPacketsReceived = 0;

char* ConvertBytesToCHexStringEx( void* inValue, const UInt32 inValueLen)
{
...
#if 0//fym
ClientSessionEx::ClientSessionEx(   UInt32 inAddr, UInt16 inPort, char* inURL,
...
:   fSocket(NULL),
	...
	fUserData(0)//fym
{
    this->SetTaskName("RTSPClientLib:ClientSessionEx");
    ...
    switch (inClientType)
    {
        ...
        default:
        {
            qtss_printf("ClientSessionEx: Attempt to create unsupported client type.\n");
            ::exit(-1);
        }
    }
...
#else
ClientSessionEx::ClientSessionEx()
:   fSocket(NULL),
	fClient(NULL),
	fTimeoutTask(this, kIdleTimeoutInMsec),
	fDurationInSec(0),
...
int ClientSessionEx::OpenSession(UInt32 inAddr, UInt16 inPort, const char* inURL, ClientType inClientType)
{
	StrPtrLen theURL((char*)inURL);
...
SInt64 ClientSessionEx::Run()
{
    ...
    if (theEvents & ClientSessionEx::kTeardownEvent)
    {
#if CLIENT_SESSION_DEBUG
    ...
    while ((theErr == OS_NoErr) && (fState != kDone))
    {
        ...
        switch (fState)
        {
                ...
                if (theErr == OS_NoErr)
                {
                    ...
                    else
                    {
                        if (fClient->IsVerbose())
                        {
                            ...
                            StrPtrLenDel theBody(ConvertBytesToCHexStringEx(fClient->GetContentBody(), receivedLength));
                            theBody.PrintStr("\n");
                        }
                        fState = kSendingDescribe;
                    }
            ...
            case kPlaying:
            {
                if(1)//fym 立即接收数据 fCurRTCPTrack == fSDPParser.GetNumStreams())
				...
				if (0)//fym ((curTime - fPlayTime) > fDurationInSec * 1000) || (fTeardownImmediately))
				{
					sPlayingConnections--;
...
void    ClientSessionEx::SetupUDPSockets()
{
    ...
    for (UInt32 x = 0; x < fSDPParser.GetNumStreams() * 2; x++)
    {
        ...
        if (theErr != OS_NoErr)
        {
            qtss_printf("ClientSessionEx: Failed to open a UDP socket.\n");
            ::exit(-1);
        }
    }
    ...
    {   
        for (UInt32 portCheck = 0; true; portCheck++)
        {
            ...
            if (portCheck == 5000)
            {
                ...
                qtss_printf("ClientSessionEx: Failed to bind a UDP socket.\n");
                ::exit(-1);
            }
        }
    }                       
...
OS_Error    ClientSessionEx::ReadMediaData()
{
...
void    ClientSessionEx::ProcessMediaPacket(  char* inPacket, UInt32 inLength,
                                            UInt32 inTrackID, Bool16 isRTCP)
{
    Assert(inLength > 4);

	//fym
	if(NULL != fDataCallBackFuncEx || 12 >= inLength)
	{
		RTPHeaderParam* hdr = (RTPHeaderParam*)inPacket;
		
		(*fDataCallBackFuncEx)((const void*)inPacket, inLength, inTrackID, fUserData);
	}
	return;
...
void ClientSessionEx::AckPackets(UInt32 inTrackIndex, UInt16 inCurSeqNum, Bool16 inCurSeqNumValid)
{
...
OS_Error ClientSessionEx::SendReceiverReport(UInt32 inTrackID)
{
    //
