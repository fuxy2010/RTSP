...
//fym char* RTSPClient::sUserAgent = "None";
...
RTSPClient::~RTSPClient()
{
    ...
    delete [] fURL.Ptr;
    //fym delete [] fName.Ptr;
    //fym delete [] fPassword.Ptr;
...
{
	//qtss_printf("\nRTSPClient::SendOptions");//fym
    if (!fTransactionStarted)
    {
        qtss_sprintf(fMethod,"%s","OPTIONS");
		//fym qtss_sprintf(fSendBuffer, "OPTIONS * RTSP/1.0\r\nCSeq:%lu\r\nUser-agent: %s\r\n\r\n", fCSeq, fUserAgent);
        qtss_sprintf(fSendBuffer, "OPTIONS %s RTSP/1.0\r\nCSeq:%lu\r\nUser-agent: %s\r\n\r\n", fURL.Ptr, fCSeq, fUserAgent);//fym
     }
    return this->DoTransaction();
}
...
OS_Error RTSPClient::SendOptionsWithRandomDataRequest(SInt32 dataSize)
{
    if (!fTransactionStarted)
    {
		...
		qtss_sprintf(fSendBuffer, "OPTIONS %s RTSP/1.0\r\nCSeq:%lu\r\nUser-agent: %s\r\nRequire: x-Random-Data-Size\r\nx-Random-Data-Size: %ld\r\n\r\n", fURL.Ptr, fCSeq, fUserAgent, dataSize);//fym
     }
    return this->DoTransaction();
}
...
{
	//qtss_printf("\nRTSPClient::SendReliableUDPSetup");//fym
...
{
	//qtss_printf("\nRTSPClient::SendRTSPRequest");//fym
    if (!fTransactionStarted)
    {
        UInt32 curOffset = 0;
...
void RTSPClient::ClearPacket(char** buf, UInt32* len)
{
	fContentLength = 0;
	fPacketBufferOffset = 0;
	fPacketOutstanding = false;
...
OS_Error    RTSPClient::GetMediaPacket(UInt32* outTrackID, Bool16* outIsRTCP, char** outBuffer, UInt32* outLen)
{
	...
	if(NULL == fPacketBuffer)
		fPacketBuffer = NEW char[kMaxPacketSize];

	if(true == fPacketOutstanding)
	{
		...
		//fym qtss_printf("<%d %d> ", thePacketLen, fPacketOutstanding);//fym

		//fym Assert(fPacketBuffer[0] == '$');
		if(fPacketBuffer[0] != '$')//fym
		{
			ClearPacket(outBuffer, outLen);
			//fPacketBufferOffset = 0;
			return OS_NoErr;
		}

		//qtss_printf("<%d %d> ", fPacketBuffer[1], 0xff * fPacketBuffer[2] + fPacketBuffer[3]);//fym

		// Move the leftover data (part of the next packet) to the beginning of the buffer
		//fym Assert(fPacketBufferOffset >= (thePacketLen + kPacketHeaderLen));
		if(fPacketBufferOffset < (thePacketLen + kPacketHeaderLen))//fym
		{
			ClearPacket(outBuffer, outLen);
			//fPacketBufferOffset = 0;
			return OS_NoErr;
	...
	{
		//fym qtss_printf("<%d> ", fPacketDataInHeaderBufferLen);//fym
		...
		//fym Assert(fPacketDataInHeaderBuffer[0] == '$');
		if(fPacketDataInHeaderBuffer[0] != '$')//fym
		{
            fPacketDataInHeaderBufferLen = 0;
            ::memset(fPacketDataInHeaderBuffer, 0, sizeof(fPacketDataInHeaderBuffer));
		}

		//qtss_printf("{%d %d} ", fPacketDataInHeaderBuffer[1], 0xff * fPacketDataInHeaderBuffer[2] + fPacketDataInHeaderBuffer[3]);//fym

		//fym Assert(fPacketDataInHeaderBufferLen < (kMaxPacketSize - fPacketBufferOffset));
		if(fPacketDataInHeaderBufferLen >= (kMaxPacketSize - fPacketBufferOffset))//fym
		{
            fPacketDataInHeaderBufferLen = 0;
            ::memset(fPacketDataInHeaderBuffer, 0, sizeof(fPacketDataInHeaderBuffer));
		}
		...
		fPacketDataInHeaderBufferLen = 0;
        ::memset(fPacketDataInHeaderBuffer, 0, sizeof(fPacketDataInHeaderBuffer));//fym
	}

	//fym Assert(fPacketBufferOffset < kMaxPacketSize);
	if(fPacketBufferOffset >= kMaxPacketSize)//fym
	{
		ClearPacket(outBuffer, outLen);
		//fPacketBufferOffset = 0;
		return OS_NoErr;
	...
	//qtss_printf("<%d %d %d> ", fPacketBufferOffset, kMaxPacketSize - fPacketBufferOffset, theRecvLen);//fym
	//fym if(theRecvLen) qtss_printf("<%d> ", theRecvLen);//fym
	...
	if (fPacketBufferOffset > kPacketHeaderLen)
	{
		...
		UInt16* thePacketLenP = (UInt16*)fPacketBuffer;//TCP数据包首地址
		UInt16 thePacketLen = ntohs(thePacketLenP[1]);//TCP数据包净荷长度

		if (fPacketBufferOffset >= (thePacketLen + kPacketHeaderLen))
		{
			// We have a complete packet. Return it to the caller.
			//fym Assert(fPacketBuffer[1] < fNumChannelElements); // This is really not a safe assert, but anyway...
			if(fPacketBuffer[1] >= fNumChannelElements)//fym
			{
				ClearPacket(outBuffer, outLen);
				return OS_NoErr;
			}

			//qtss_printf("(T %d) ", fPacketBuffer[1]);//fym
	...
	//qtss_printf("%d ", fPacketBufferOffset);
	return OS_NoErr;
}
...
OS_Error RTSPClient::DoTransaction()
{
    ...
    /*fym if (fAuthenticator != NULL && !fTransactionStarted) 
    {   
    ...
    while (!fReceivedResponse)
    {
        ...
        {               
            ...
            while (theParser.GetDataRemaining() > 0)
            {
                ...
                /*fym else if (theKey.NumEqualIgnoreCase(sAuthenticateHeader.Ptr, sAuthenticateHeader.Len))
                {   
                    if (fVerbose)
                        qtss_printf("\n--CHALLENGE RECEIVED\n");
