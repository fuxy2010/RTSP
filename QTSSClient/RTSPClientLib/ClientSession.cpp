...
ClientSession::ClientSession(UInt32 inAddr, UInt16 inPort, char* inURL, ClientType inClientType)
:   fSocket(NULL),
	fClient(NULL),
	fTimeoutTask(this, kIdleTimeoutInMsec),
	fDurationInSec(0),
	fStartPlayTimeInSec(0),
	fRTCPIntervalInSec(5000),
	fOptionsIntervalInSec(0),
	fOptions(true),
	fOptionsRequestRandomData(false),
	fOptionsRandomDataSize(0),
	...
	fAppendJunk(false),
	fReadInterval(10),
	fSockRcvBufSize(32768),
	fSpeed(1),
	fPacketRangePlayHeader(NULL),
	fStats(NULL),
	fOverbufferWindowSizeInK(5120),
	fCurRTCPTrack(0),
	fNumPacketsReceived(0),
	fDataCallBackFunc(NULL),//fym
	fUserData(0),//fym
	fFrameBufOffSet(0),//fym
	fRecvData(false),//fym
	fLastSendOptionTime(0),//fym
	fSendOptionIntervalInSec(25),//fym
	fClientType(inClientType),//fym
	fRTPHeaderLen(sizeof(RTPHeaderParam))//fym
	//fDataCallBackFuncEx(NULL),//fym
	//fdwUserData1(0),//fym
	//fdwUserData2(0)//fym
{
    ...
    if (1)//fym true == sendOptions)
       fState = kSendingDescribe;//fym kSendingOptions;
		...
		case kRTSPHTTPClientType:
			{
				...
				fTransportType = kTCPTransportType;
				fSocket = NEW HTTPClientSocket(theURL, 10000, Socket::kNonBlockingSocketType);
				break;
			}
    ...
    fClient = NEW RTSPClient(fSocket, 0);
    fClient->Set(theURL);
    fClient->SetSetupParams(0, NULL);
...
SInt64 ClientSession::Run()
{
    ...
    if (theEvents & Task::kStartEvent)
    {
        ...
        return 1;//fym ::rand() % kMaxWaitTimeInMsec;
    }
    ...
    while ((theErr == OS_NoErr) && (fState != kDone))
    {
        ...
        switch (fState)
        {
            ...
            case kPlaying:
            {
				if (1)//fym fCurRTCPTrack == fSDPParser.GetNumStreams())
				...
				//fym
				if((curTime - fLastSendOptionTime) > (fSendOptionIntervalInSec * 1000))
				{
					if(NULL != fClient && kTCPTransportType != fTransportType)//fym TCP模式时不发OPTIONS避免影响接受媒体数据流
					{
						//qtss_printf("\noption!");//fym
						fClient->SendOptions();
					}

					fLastSendOptionTime = curTime;
				}
				//fym 自动断开
				/*if(fTeardownImmediately)//fym if (((curTime - fPlayTime) > fDurationInSec * 1000) || (fTeardownImmediately))
				{
				sPlayingConnections--;
				...
				return fReadInterval;//fym 否则返回值为0导致CPU占用率过高 break;
            }
...
{
	static UInt16 sCurrentRTPPortToUse = 16970;//fym
    static const UInt16 kMinRTPPort = 16970;//fym 6970;
    static const UInt16 kMaxRTPPort = 56970;
    ...
    {   
        for (UInt32 portCheck = 0; true; portCheck++)
        {
			...
			if(portCheck)
			{
				qtss_printf("%d %d, ", portCheck, sCurrentRTPPortToUse);//fym
			}
...
OS_Error    ClientSession::ReadMediaData()
{
    ...
    while(true)
    {
        ...
        //qtss_printf("%d %d, ", theLength, fTransportType);//fym
        //
...
{
    //fym Assert(inLength > 4);
	fRecvData = true;//fym

	if (isRTCP)
		return;

	UInt64 theTimeStampSSRC = 0;

	if(1)/*if(kRTSPTCPClientType == fClientType)
	{

	}
	else if(kRTSPUDPClientType == fClientType)*/
	{
		if(12 >= inLength)
		{
			qtss_printf("\nWrong packet!");
		...
		RTPHeaderParam* hdr = (RTPHeaderParam*)inPacket;
		//theTimeStampSSRC = ntohl(hdr->TIME_STAMP);
		//theTimeStampSSRC <<= 32;
		//theTimeStampSSRC |= ntohl(hdr->SSRC);

		if(fFrameBufOffSet + inLength - fRTPHeaderLen > sizeof(fFrameBuf))
		{
			qtss_printf("\nFrame buffer is not big enough!");
			fFrameBufOffSet = 0;
			return;
		}

		::memcpy(fFrameBuf + fFrameBufOffSet, inPacket + fRTPHeaderLen, inLength - fRTPHeaderLen);
		fFrameBufOffSet += (inLength - 12);

		if(hdr->M)
		{
			if(NULL != fDataCallBackFunc)
			{
				//(*fDataCallBackFunc)((unsigned char*)fFrameBuf, fFrameBufOffSet, inTrackID, hdr->M, ntohs(hdr->SEQ_NUM), &theTimeStampSSRC, fUserData);
				(*fDataCallBackFunc)((unsigned char*)fFrameBuf, fFrameBufOffSet, inTrackID, ntohl(hdr->SSRC), fUserData);
			}
...
void ClientSession::AckPackets(UInt32 inTrackIndex, UInt16 inCurSeqNum, Bool16 inCurSeqNumValid)
{
    ...
    *(theWriter++) = htonl(0);
    ...
    // Send the packet
    //fym Assert(fStats[inTrackIndex].fDestRTCPPort != 0);
	if(!(fStats[inTrackIndex].fDestRTCPPort))//fym
		return;
...
#define OB 0x6F620004
    ...
    *(theWriter++) = htonl(0);      // should be a short, but we need to pad to a long for the entire RTCP app packet
    ...
    if (fUDPSocketArray != NULL)
    {
        //fym Assert(fStats[inTrackID].fDestRTCPPort != 0);
