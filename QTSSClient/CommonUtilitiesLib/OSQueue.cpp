...
{
	//fym OSMutexLocker theLocker(&fMutex);//fym

    //fym Assert(elem != NULL);
	if(NULL == elem)//fym
		return;
    ...
    //fym Assert(elem->fQueue == NULL);
	if(NULL != elem->fQueue)//fym
		return;

    elem->fNext = fSentinel.fNext;
    elem->fPrev = &fSentinel;
    elem->fQueue = this;
    fSentinel.fNext->fPrev = elem;//保证哨兵的prev永远指向队列head
    fSentinel.fNext = elem;//保证哨兵的next永远指向队列tail
    fLength++;
}
...
OSQueueElem* OSQueue::DeQueue()
{
    ...
    if (fLength > 0)
    {
        ...
        //fym Assert(fSentinel.fPrev != &fSentinel);
		if(fSentinel.fPrev == &fSentinel)//fym
			return NULL;
...
void OSQueue::Remove(OSQueueElem* elem)
{
    ...
    //fym Assert(elem != NULL);
    //fym Assert(elem != &fSentinel);
	if(NULL == elem || elem == &fSentinel)//fym
		return;
