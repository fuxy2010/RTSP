...
UDPSocket::SendTo(UInt32 inRemoteAddr, UInt16 inRemotePort, void* inBuffer, UInt32 inLength)
{
	...
	//qtss_printf("#");//fym
	//in_addr addr;//fym
	//addr.S_un.S_addr = inRemoteAddr;//fym
...
char ts[] = "aaaaaaaaaaaaaaaaaaaaaaaaaaa";//fym
                            ...
                            void* ioBuffer, UInt32 inBufLen, UInt32* outRecvLen)
{
	...
	//fym
	//ZeroMemory(ioBuffer, inBufLen);
	//memcpy(ioBuffer, ts, sizeof(ts));
	//theRecvLen = sizeof(ts);
    ...
    *outRemotePort = ntohs(fMsgAddr.sin_port);
	//qtss_printf("\nMsgAddr.sin_port: %d", fMsgAddr.sin_port);//fym
    Assert(theRecvLen >= 0);
