...
#define DESTROY_WINDOW_MESSAGE	WM_USER+1024
...
int select_waitevent(struct eventreq *req, void* /*onlyForMacOSX*/)
{
    if (sMsgWindow == NULL)
    {
		...
		char theWndName[32];
		::ZeroMemory(theWndName, sizeof(theWndName));
		qtss_sprintf(theWndName, "RTSPClient%d", ::time(NULL));
        ...
        theWndClass.lpszClassName = theWndName;//fym "DarwinStreamingServerWindow";
        ...
        sMsgWindow = ::CreateWindow(    theWndName, /*fym "DarwinStreamingServerWindow",*/  // Window class name
                                        theWndName, /*fym "DarwinStreamingServerWindow",*/  // Window title bar
	...
	//fym
	if(DESTROY_WINDOW_MESSAGE == theMessage.message)
	{
		//qtss_printf("\nDestroy windows!");

		::DestroyWindow(sMsgWindow);
		sMsgWindow = NULL;
		return 128;//fym 令线程销毁
	}
...
//fym
int select_closeevent()
{
	qtss_printf("\nselect_closeevent");

	if(NULL != sMsgWindow)
	{
		qtss_printf("\npost destroy window message!");
		//fym 将DESTROY_WINDOW_MESSAGE定义为关闭窗口消息
		::PostMessage(sMsgWindow, DESTROY_WINDOW_MESSAGE, 0, 0);//fym 不可用SendMessage
	}
