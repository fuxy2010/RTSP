...
:   QTSSDictionary(QTSSDictionaryMap::GetMap(QTSSDictionaryMap::kRTPStreamDictIndex), NULL),
    ...
    fStreamStartTimeOSms(OS::Milliseconds()),//fym
	fStopWrite(false)//fym
{
    ...
    //fym Assert(fSsrcStringPtr.Len < kMaxSsrcSizeInBytes);
...
RTPStream::~RTPStream()
{
    ...
    if(fSockets != NULL)
    {
        ...
        //fym Assert(fSockets->GetSocketB()->GetDemuxer() != NULL);
		if(NULL == fSockets->GetSocketB()->GetDemuxer())//fym
			return;
...
QTSS_Error RTPStream::Setup(RTSPRequestInterface* request, QTSS_AddStreamFlags inFlags)
{
    ...
    if(fTransportType == qtssRTPTransportTypeTCP)
    {
		...
		//一个RTSPSession的每路数据流分配两个Channel，用于RTP/RTCP
...
void    RTPStream::AppendRTPInfo(QTSS_RTSPHeader inHeader, RTSPRequestInterface* request, UInt32 inFlags, Bool16 lastInfo)
{
    ...
    if(inFlags & qtssPlayRespWriteTrackInfo)
    {
        ...
        //fym Assert(rtpTimeBufPtr.Len < 20);
    }   
    ...
    if(inFlags & qtssPlayRespWriteTrackInfo)
    {
        ...
        //fym Assert(seqNumberBufPtr.Len < 20);
    }
...
QTSS_Error  RTPStream::InterleavedWrite(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, unsigned char channel)
{
	...
	if(qtssPausedState == fSession->GetSessionState() || NULL == fSession->GetRTSPSession())//fym
		return QTSS_NoErr;

    //char blahblah[2048];

	/*fym if(2 == fSession->GetRTSPSession()->fStatus)//fym
		return QTSS_NoErr;*/
	...
	if(qtssPausedState == fSession->GetSessionState())//fym
		return QTSS_NoErr;
...
//      qtss_printf("Flow controlled\n");
    ...
    else
    {
		...
		//fym RTPStream::Write->RTPStream::ReliableRTPWrite
...
{
    //fym Assert(fNumQualityLevels > 0);
	if(fNumQualityLevels <= 0)//fym
		return false;
...
{
	OSMutexLocker(this->GetMutex());//fym
	if(true == fStopWrite)//fym
		return QTSS_NoErr;

	//qtss_printf("%ul ", this);//fym
    //fym Assert(fSession != NULL);
	if(NULL == fSession)//fym
		return QTSS_NoErr;
	...
	/*fym if(qtssPausedState == fSession->GetSessionState() || NULL == fSession->GetRTSPSession())//fym
		return QTSS_NoErr;

	if(2 == fSession->GetRTSPSession()->fStatus)//fym
		return QTSS_NoErr;*/
    ...
    {   
		...
		if(false == *(fSession->GetOverbufferWindow()->OverbufferingEnabledPtr())) // only force rtcps on time if overbuffering is off
		{
            ...
            {
                //fym Assert(thePacket->suggestedWakeupTime >= fSession->GetOverbufferWindow()->GetSendInterval());			
