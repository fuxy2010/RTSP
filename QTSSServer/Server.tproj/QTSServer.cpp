...
void QTSServer::InitModules(QTSS_ServerState inEndState)
{
	...
	//fym LoadModules(fSrvrPrefs);//动态加载模块，即寻找QTSSModules目录下的动态链接库并加载
    LoadCompiledInModules();//静态加载模块
    this->BuildModuleRoleArrays();
...
//fym 从配置文件streamingserver.xml的"rtsp_port"字段中获取要侦听的RTSP端口数
