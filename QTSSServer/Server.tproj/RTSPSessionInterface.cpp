...
UInt8 RTSPSessionInterface::GetTwoChannelNumbers(StrPtrLen* inRTSPSessionID)
{
    ...
    {
        //fym Assert(numChannelEntries > 1);
		if(numChannelEntries <= 1)//fym
...
QTSS_Error RTSPSessionInterface::InterleavedWrite(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, unsigned char channel)
{
    ...
    if( err == QTSS_NoErr )
    {
        ...
        if( inLen > kTCPCoalesceDirectWriteSize )  
        {
            ...
            rih.header = '$';
            rih.channel = channel;//一个RTSPSession的每路数据流分配两个Channel，用于RTP/RTCP
            rih.len = htons( (UInt16)inLen);

			//qtss_printf("<%d %d> ", channel, inLen);//fym
...
{
    //fym Assert( fromRTSPSession != NULL );
    //fym Assert( fromRTSPSession->fOutputSocketP != NULL );
	if(NULL == fromRTSPSession)//fym
		return;
	if(NULL  == fromRTSPSession->fOutputSocketP)//fym
		return;
...
{
	//fym Assert(fOldOutputStreamBuffer.Ptr == NULL);
...
{
	//fym Assert(fOldOutputStreamBuffer.Ptr != NULL);
	//fym Assert(fOldOutputStreamBuffer.Len != 0);
