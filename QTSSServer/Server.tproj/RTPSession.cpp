...
RTPSession::~RTPSession()
{
    ...
    if(QTSServerInterface::GetServer()->GetPrefs()->GetReliableUDPPrintfsEnabled())
    {
        ...
        {
            //fym Assert(theStream != NULL);
            //fym Assert(theLen == sizeof(RTPStream*));
			if(NULL == theStream || theLen != sizeof(RTPStream*))//fym
				continue;
            if(*theStream != NULL)
            {
    ...
    
    {
        ...
        //fym Assert(y < theServer->GetNumRTPSessions());
		if(y >= theServer->GetNumRTPSessions())//fym
			return;
...
QTSS_Error  RTPSession::Activate(const StrPtrLen& inSessionID)
{
    ...
    //fym Assert(inSessionID.Len <= QTSS_MAX_SESSION_ID_LENGTH);
	if(inSessionID.Len > QTSS_MAX_SESSION_ID_LENGTH)//fym
...
{
    //fym Assert(outStream != NULL);
	if(NULL == outStream)//fym
		return QTSS_RequestFailed;
...
//准备好需要打包发送的数据后，利用Task类的Signal函数传信RTPSession对象，使其被加入某个TaskThread的任务队列，从而运行其Run函数
...
//  qtss_printf("movie bitrate = %d, window size = %d\n", this->GetMovieAvgBitrate(), theWindowSize);
    //fym Assert(this->GetBandwidthTracker()->BytesInList() == 0);
    ...
    //fym Assert(fRTSPSession != NULL); // can this ever happen?
...
void RTPSession::Teardown()
{
    ...
    // an RTP stream.
#ifdef _DEBUG//fym
	qtss_printf("\nRTPSession::Teardown");//fym
#endif
	fState = qtssPausedState;//fym 提前
	...
	for (int x = 0; this->GetValuePtr(qtssCliSesStreamObjects, x, (void**)&theStream, &theLen) == QTSS_NoErr; x++)
	{
		if(NULL != theStream && theLen == sizeof(RTPStream*))//fym
		{
			if(NULL != *theStream)
			{
				OSMutexLocker((*theStream)->GetMutex());
				(*theStream)->fStopWrite = true;
			}
		}
	}

	if(NULL != fRTSPSession)//fym
	{
		fRTSPSession->fStatus = 2;
...
SInt64 RTPSession::Run()
{
    ...
    //fym if((events & Task::kKillEvent) || /*fym 不因超时关闭Session (events & Task::kTimeoutEvent) ||*/ (fModuleDoingAsyncStuff))
        ...
        if(!fModuleDoingAsyncStuff)
    {
        ...
        //fym Assert(sessionTable != NULL);
		if(NULL == sessionTable)//fym
			return 0;
        ...
        UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kClientSessionClosingRole);
        {
            ...
            {  
				...
				//fym 客户端异常终止后居然过150秒左右才触发该事件
				/*if(events & Task::kTimeoutEvent)
				{
					if(!strcmp("QTSSReflectorModule", theModule->GetPrefsDict()->GetPrefName()))
						//return 0;
					qtss_printf("\n%s is time out!", theModule->GetPrefsDict()->GetPrefName());
				}*/

                //fym (void)theModule->CallDispatch(QTSS_ClientSessionClosing_Role, &theParams);
				if(events & Task::kTimeoutEvent)
				{
					qtss_printf("\n%s is timeout!", theModule->GetPrefsDict()->GetPrefName());
				}
    ...
    //fym Assert(theParams.rtpSendPacketsParams.outNextPacketTime >= 0);//we'd better not get deleted accidently!
	if(theParams.rtpSendPacketsParams.outNextPacketTime < 0)//fym
		return 0;
