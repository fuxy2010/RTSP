...
QTSS_Error RTSPRequest::Parse()
{
    ...
    //fym Assert(this->GetValue(qtssRTSPReqFullRequest)->Ptr != NULL);
	if(NULL == this->GetValue(qtssRTSPReqFullRequest)->Ptr)//fym
...
QTSS_Error RTSPRequest::ParseHeaders(StringParser& parser)
{
    ...
    while ((parser.PeekFast() != '\r') && (parser.PeekFast() != '\n')) 
    {
        ...
        {
            //fym Assert(theHeader < qtssNumHeaders);
			if(theHeader >= qtssNumHeaders)//fym
				return QTSS_RequestFailed;
