...
class ReflectorSession
{
    public:
        ...
        ReflectorStream* GetStreamByIndex(UInt32 inIndex)
		{
			//fym 避免在fStreamArray还未创建前就发包导致的问题 return fStreamArray[inIndex];
			if(NULL != fStreamArray)
	   ...
	   static void RemoveStreamMap();//fym for leak
