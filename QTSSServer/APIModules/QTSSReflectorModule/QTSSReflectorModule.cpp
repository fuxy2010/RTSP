...
static StrPtrLen    sSDPSuffix(".sdp");//fym 如果客户端传来的请求中的URL不以.sdp结尾，则强行在URL末尾加上此后缀
...
//static OSQueue*     sReflectorSessionQueue = NULL;//fym 暂时不用，ReflecorSession的维护由sSessionMap管理

//fym
static OSMutex* sSessionMutexArray[65536];
static UInt16	sSessionStatus[65536];//fym 0-未生成, 1-已生成, 2-即将被销毁
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params* inParams)
{
	...
	QTSSCharArrayDeleter theFullPathStrDeleter(theFullPathStr);//fym 务必和QTSS_GetValueAsString配对使用否则导致内存泄漏!!!
...
Bool16 CheckSDPPath(char* sdp_path)//fym sdp_path必须是"\0.sdp"～"\65535.sdp"
{
	if(NULL == sdp_path)
		return false;
...
// ------------  Clean up missing required SDP lines
    ...
    //fym 以下语句即可替换该函数执行 DoAnnounceAddRequiredSDPLines(inParams, &editedSDP, theRequestBody);
...
//这样在QTSSReflectorModule::DoSetup函数里，会再次创建RTPSessionOutput对象，并添加到ReflectorStream的fOutputArray数组里。同时也会调用QTSS_AddRTPStream函数。
...
{   
    ...
    OSRef* theSessionRef = sSessionMap->Resolve(inPath);//fym 第一次执行此语句，因ReflectorSession未创建故引用计数不增加
    ReflectorSession* theSession = NULL;
...
void RemoveOutput(ReflectorOutput* inOutput, ReflectorSession* inSession, Bool16 killClients)
{
    ...
    if(inSession != NULL)
    {
...
#if 0//fym 修改销毁ReflectorSession的模式
...
#else//fym 修改销毁ReflectorSession的模式
		...
		if(!(inSession->GetNumOutputs()))//fym if(0)//fym 不得已以永不销毁ReflectorSession为代价换取稳定性! (inSession->GetNumOutputs()))
		{
			...
			//fym提前 UInt16 uuid  = ResolveUUIDFromPath(inSession->GetSourcePath()->Ptr);//fym

			//fym提前 OSMutexLocker locker(sSessionMutexArray[uuid]);
			...
			if(NULL != inSession)//fym 确保如果有其他线程删除了此Session不会引起异常
			{
...
#endif//fym 修改销毁ReflectorSession的模式
    }
...
ReflectorSession* FindExistingReflectorSession(UInt16 uuid)
{
	...
	qtss_sprintf(temp, "\\%d.sdp", uuid);//fym 务必加"\\"，否则有重名sdp文件 qtss_sprintf(temp, "SDP\\%d.sdp", uuid);
