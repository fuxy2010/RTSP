...
{
    //fym Assert(fNumStreams > 0);
	if(0 >= fNumStreams)//fym
		return;
    ...
    if(isRTSPSourceInfo)
    {
        ...
        //fym Assert(rtspSourceInfo != NULL);
		if(NULL == rtspSourceInfo)//fym
			return;
        RTSPSourceInfo* sessionSourceInfo = (RTSPSourceInfo *)(inRelaySession->GetSourceInfo());
        //fym Assert(sessionSourceInfo != NULL);
		if(NULL == sessionSourceInfo)//fym
			return;
        ...
        if(rtspInfo->fIsAnnounced)
        {
            ...
            //fym fClient->SetName(rtspInfo->fUserName);
            //fym fClient->SetPassword(rtspInfo->fPassword);
...
RelayOutput::~RelayOutput()
{
    ...
    {
        //fym Assert(outputObject != NULL);
        //fym Assert(len == sizeof(QTSS_Object));
		if(NULL == outputObject || len != sizeof(QTSS_Object))//fym
			continue;
...
QTSS_Error  RelayOutput::WritePacket(StrPtrLen* inPacket, void* inStreamCookie, UInt32 inFlags, SInt64 /*packetLatenessInMSec*/, SInt64* /*timeToSendThisPacketAgain*/, UInt64* packetIDPtr, SInt64* /*arrivalTimeMSec*/ )
{
    ...
    for (UInt32 x = 0; x < fNumStreams; x++)
    {
        if(inStreamCookie == fStreamCookieArray[x])
        {
            ...
            //fym Assert((theDestPort & 1) == 0); //this should always be an RTP port (even)
			if(0 != (theDestPort & 1))//fym
				continue;
			...
			//qtss_printf("+");//fym
			//in_addr addr;//fym
			//addr.S_un.S_addr = fOutputInfo.fDestAddr;//fym
			//qtss_printf(": %s ", inet_ntoa(addr));//fym
