...
:   fSlidingWindow(NULL),
    ...
    fHighestSeqNumber(0)
{
    //fym Assert(fNegativeWindowSize < 0);
    //fym Assert(fWindowSize < 32768);//AddSequenceNumber makes this assumption
...
Bool16 SequenceNumberMap::AddSequenceNumber(UInt16 inSeqNumber)
{
    ...
    //fym Assert(theWindowIndex >= 0);
	if(theWindowIndex < 0)//fym
		return false;
    //fym Assert(theWindowIndex < fWindowSize);
	if(theWindowIndex >= fWindowSize)//fym
		return false;
