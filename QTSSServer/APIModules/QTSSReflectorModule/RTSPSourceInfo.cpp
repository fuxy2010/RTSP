...
RTSPSourceInfo::~RTSPSourceInfo()
{
    ...
    {
        //fym Assert(fClientSocket != NULL);
        //fym Assert(fClient != NULL);
        // the task will delete these objects when it is done with the teardown
        //fym TeardownTask* task = new TeardownTask(fClientSocket, fClient);
        //fym task->Signal(Task::kStartEvent);
		if(NULL != fClientSocket && NULL != fClient)//fym
		{
    ...
    if(fAnnounceURL != NULL)
	{
		delete fAnnounceURL;
		fAnnounceURL = NULL;//fym
	}
    
    if(fRTSPInfoArray != NULL)
	{
		delete [] fRTSPInfoArray;
		fRTSPInfoArray = NULL;//fym
	}
}
...
void RTSPSourceInfo::InitClient(UInt32 inSocketType)
{
    ...
    fClient = NEW FakeRTSPClient(fClientSocket, false, RelaySession::sRelayUserAgent);
}
...
void RTSPSourceInfo::AddRelayDestinations(char* src_ip, UInt16 src_port, char* src_url, char* dest_url)
{
	//replace of QTSS_Error theErr = theRTSPInfo->ParsePrefs(relayTag, false);//转发源
	fAnnounce = false;

	if(NULL != src_ip)//"PREF", "NAME", "source_addr"
	{
		fHostAddr = SocketUtils::ConvertStringToAddr(src_ip);
	}

	fHostPort = src_port;//"PREF", "NAME", "rtsp_port"

	if(NULL != src_url)//"PREF", "NAME", "url"
	{
		fSourceURL = RTSPOutputInfo::CopyString(src_url);
	}



	//fym 以下两行语句替代该函数 ParseRelayDestinations(relayTag);//转发URL
	AllocateOutputArray(1);//一个转发URL
	fNumOutputs = 1;

	//fym 以下至结尾语句替代该函数 ParseAnnouncedDestination(destTag, y);
	int index = 0;//

	fRTSPInfoArray[index].fIsAnnounced = true;

	fOutputArray[index].fDestAddr = SocketUtils::ConvertStringToAddr("127.0.0.1");//"PREF", "NAME", "dest_addr"

	fRTSPInfoArray[index].fAnnouncePort = 554;//"PREF", "NAME", "rtsp_port"

	if(NULL != dest_url)//"PREF", "NAME", "url"
	{
		fRTSPInfoArray[index].fDestURl = RTSPOutputInfo::CopyString(dest_url);
	}
}
...
SInt64 RTSPSourceInfo::RunCreateSession()
{
	...
	if(fSessionCreationState == kSendingDescribe)
	{
		if(!fDescribeComplete)
		{
			//fym fake osErr = fClient->SendDescribe();
			//fClient->SendDescribe();//fym fake
			osErr = OS_NoErr;//fym fake


			if(osErr == OS_NoErr)
			{
				if(1)//fym fake fClient->GetStatus() == 200)
				{
					// we've gotten the describe response, so process it.
					//fym fake SDPSourceInfo theSourceInfo(fClient->GetContentBody(), fClient->GetContentLength());
					SDPSourceInfo theSourceInfo(temp_describe, sizeof(temp_describe));
		...
		if(osErr == OS_NoErr)
		{
			...
			/*fym fake
			把fClient的ContentBody除range行外整个复制到fLocalSDP中
			fLocalSDP.Ptr = NEW char[fClient->GetContentLength() + 1];
			...
			fLocalSDP.Ptr = NEW char[sizeof(temp_describe) + 1];//fym fake
			::memset(fLocalSDP.Ptr, 0, sizeof(temp_describe) + 1);//fym fake
			::memcpy(fLocalSDP.Ptr, temp_describe, sizeof(temp_describe));//fym fake
			fLocalSDP.Len = sizeof(temp_describe);//fym fake
...
#define _WRITE_SDP_ 0
	...
	{
		//fClient->SendUDPSetup(fStreamArray[fNumSetupsComplete].fTrackID, fStreamArray[fNumSetupsComplete].fPort);
		//qtss_printf("\nfStreamArray: %d\n", fNumSetupsComplete);//fym
		osErr = OS_NoErr;//fym fake fClient->SendUDPSetup(fStreamArray[fNumSetupsComplete].fTrackID, fStreamArray[fNumSetupsComplete].fPort);
		if(osErr == OS_NoErr)
		{
			if(1)//fym fake fClient->GetStatus() == 200)
			{
	...
	{
		//fClient->SendPlay(0);
		osErr = OS_NoErr;//fym fake fClient->SendPlay(0);
		if(osErr == OS_NoErr)
		{
			if(1)//fym fake fClient->GetStatus() == 200)
...
RTSPSourceInfo::TeardownTask::TeardownTask(TCPClientSocket* clientSocket, FakeRTSPClient* client)
{
