...
void QueryURI::ParseQueryString(StringParser *parserPtr,StrPtrLen *urlStreamPtr)
{
    ...
    {   
        ...
        if(*stopCharPtr == '"') // if quote read to next quote
        {
            ...
            tempQueryParse.ConsumeLength(NULL, 1);
            //qtss_printf("QueryURI::ParseQueryString is quote GetCurrentPosition = '%s' len = %lu\n",stopCharPtr, strlen(stopCharPtr));
        }
        else
        {
            //qtss_printf("QueryURI::ParseQueryString white or EOL GetCurrentPosition = '%s' len = %lu\n",stopCharPtr, strlen(stopCharPtr));
            if(*stopCharPtr == ' ') 
            {   tempQueryParse.ConsumeWhitespace();
                continue;
            }
    ...
    //qtss_printf("Query String = '%s' Query len = %lu parseLen = %lu\n",queryStr.Ptr, queryStr.Len,len);
...
{   
    ...
    //qtss_printf("QueryURI::ParseURLString fURIBuffer =%s len = %lu\n", fURIBuffer,urlStreamPtr->Len);
...
{   
    if(inStream != NULL) 
    {
        ...
        { 
            ...
            {   
                //qtss_printf("URL string bigger than Buffer size=%lu\n",decodedRequestStr.Len);
                break;
            }       
