...
static UInt16 sCurLowestPort = UDPSocketPool::kLowestUDPPort;//fym necessary
...
UDPSocketPair*  UDPSocketPool::CreateUDPSocketPair(UInt32 inAddr, UInt16 inPort)
{
  //try to find an open pair of ports to bind these suckers tooo
    ...
    UInt16 curPort = sCurLowestPort;//fym necessary kLowestUDPPort;
    ...
    while ((!foundPair) && (curPort < kHighestUDPPort))
    {
		...
		//qtss_printf("\nudp pair: %d, %d", curPort, socketBPort);//fym
        OS_Error theErr = theElem->fSocketA->Bind(inAddr, curPort);
        if(theErr == OS_NoErr)
        {   //fym qtss_printf("fSocketA->Bind ok on port%u\n", curPort);
            theErr = theElem->fSocketB->Bind(inAddr, socketBPort);
            if(theErr == OS_NoErr)
            {   //fym qtss_printf("fSocketB->Bind ok on port%u\n", socketBPort);
                foundPair = true;
                fUDPQueue.EnQueue(&theElem->fElem);
                theElem->fRefCount++;

				sCurLowestPort += 2;//fym necessary
				sCurLowestPort = (sCurLowestPort < (kHighestUDPPort - 10000)) ? sCurLowestPort : kLowestUDPPort;//fym necessary 端口循环使用
		...
		//fym necessary 实际进入ReflectorSocketPool::DestructUDPSocketPair
