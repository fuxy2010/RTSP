...
//每个TimeoutTask创建时都会将自身添加到TimeoutTaskThread（单键）的队列fQueue中
//同时制定该TimeoutTask超时的时间
...
SInt64 TimeoutTaskThread::Run()
{
	...
	for (OSQueueIter iter(&fQueue); !iter.IsDone(); iter.Next())
	{
		...
		//如果当前TimeoutTask对象的超时时戳大于0，且当前时间已经大于当前TimeoutTask对象的超时时戳，则对触发当前TimeoutTask对象对应的Task对象的超时事件
		//qtss_printf("\n%16s %I64u %I64u", theTimeoutTask->GetTask()->fTaskName, (theTimeoutTask->fTimeoutAtThisTime) % 1000000, curTime % 1000000);//fym
...
#endif
			//qtss_printf("\n%16s timeout!", theTimeoutTask->GetTask()->fTaskName);//fym
			theTimeoutTask->fTask->Signal(Task::kTimeoutEvent);
		}
		else
		{
			taskInterval = theTimeoutTask->fTimeoutAtThisTime - curTime;//计算当前时间距该TimeoutTask对象超时时戳的剩余时间

			if((taskInterval > 0)//剩余时间大于0
				&& (theTimeoutTask->fTimeoutInMilSecs > 0)//该TimeoutTask对象的超时时间大于0
				&& (intervalMilli > taskInterval))//下次运行本函数前该TimeoutTask对象已超时
			{
				//令TimeoutTaskThread的下次在该TimeoutTask对象超时后1秒即运行
