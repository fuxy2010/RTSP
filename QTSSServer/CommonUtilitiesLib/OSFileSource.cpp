...
{ 
    ...
    if((!fFileMap.Initialized()) || (!fCacheEnabled) || (fFileMap.GetBuffIndex(inPosition+inLength) > fFileMap.GetMaxBuffIndex()))
	{
...
{ 
   ...
   if(inPosition >= fLength) // eof
	...
	qtss_printf("]", inLength);//fym
	//qtss_printf("\nbytesToCopy %d", inLength);//fym
